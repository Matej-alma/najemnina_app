...
function expenses_list(PDO $pdo, int $userId, int $apartmentId): void {
  ...
  if (!$st->fetch()) json_response(404, ['error' => 'Apartment not found']);
  ...
  json_response(200, ['items' => $q->fetchAll()]);
}
...
function expenses_get(PDO $pdo, int $userId, int $expenseId): void {
  ...
  $row = $st->fetch();
  if (!$row) json_response(404, ['error' => 'Expense not found']);
...
function expenses_create(PDO $pdo, int $userId, int $apartmentId): void {
  ...
  if ($date === '' || $type === '' || $amount === null) {
    json_response(422, ['error' => 'Missing fields: expense_date, type, amount']);
  }
  if (!is_numeric($amount) || (float)$amount <= 0) {
    json_response(422, ['error' => 'amount must be > 0']);
  }

  $ins = $pdo->prepare('INSERT INTO expenses (apartment_id, expense_date, type, amount, description)
                        VALUES (?, ?, ?, ?, ?)');
...
function expenses_update(PDO $pdo, int $userId, int $expenseId): void {
  ...
  if (!$chk->fetch()) json_response(404, ['error' => 'Expense not found']);
...
function expenses_delete(PDO $pdo, int $userId, int $expenseId): void {
  ...
  ');
  $st->execute([$expenseId, $userId]);
  if ($st->rowCount() === 0) json_response(404, ['error' => 'Expense not found']);
